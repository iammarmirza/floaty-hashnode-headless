        ...
        <div className="flex flex-wrap gap-3 md:gap-4">
            {
                links.map(([socialName, socialLink]) => (
                    <a href={socialLink as string} target="_blank" className="flex p-2 rounded-full hover:shadow-sm hover:bg-slate-600 hover:text-white" key={socialName}>
                        <FontAwesomeIcon icon={SOCIAL_LINKS[socialName]} className="h-4 w-4 md:h-5 md:w-5" />
                    </a>
                ))
            }
        </div>
